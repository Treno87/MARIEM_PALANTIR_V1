...
import {
	...
	useCustomersList,
	useUpdateCustomer,
} from "../../hooks/useCustomersApi";
import { USE_API } from "../../lib/config";
import { mapApiCustomerToCustomer } from "../../utils/apiMappers";
...
export default function CustomersPage(): ReactElement {
	...
	// API 읽기 훅 (USE_API=true이고 인증된 상태일 때만 fetch)
	const { isAuthenticated } = useAuth();
	const customersQuery = useCustomersList(undefined, {
		enabled: USE_API && isAuthenticated,
	});
	const apiCustomers = useMemo((): Customer[] => {
		if (!USE_API || !customersQuery.data) return [];
		return customersQuery.data.map(mapApiCustomerToCustomer);
	}, [customersQuery.data]);

	// 실제 사용할 고객 데이터
	const customers = USE_API ? apiCustomers : contextCustomers;
	...
	const [statFilter, setStatFilter] = useState<"all" | "vip" | "active" | "dormant">("all");

	const filteredCustomers = customers.filter((c) => {
		// 상태 필터
		if (!showInactive && c.status === "inactive") return false;
		// 스탯 카드 필터
		if (statFilter === "vip" && getCustomerTier(c.totalSpent) !== "vip") return false;
		if (statFilter === "active" && getAutoStatus(c.lastVisitDate) !== "active") return false;
		if (statFilter === "dormant" && getAutoStatus(c.lastVisitDate) !== "inactive") return false;
		// 검색 필터
		if (searchQuery.trim() !== "") {
	...
	const formatLtv = (amount: number): string => {
		if (amount >= 100_000_000) {
			const eok = Math.floor(amount / 100_000_000);
			const man = Math.floor((amount % 100_000_000) / 10_000);
			return man > 0 ? `${String(eok)}억 ${man.toLocaleString()}만원` : `${String(eok)}억원`;
		}
		if (amount >= 10_000) {
			...
			<div className="shrink-0 px-8 pt-8">
				...
				<div className="mb-6 grid grid-cols-5 gap-4">
					<button
						type="button"
						onClick={() => {
							setStatFilter(statFilter === "all" ? "all" : "all");
						}}
						className={`rounded-xl border p-4 text-left transition-colors ${
							statFilter === "all"
								? "border-neutral-400 bg-neutral-50 ring-1 ring-neutral-400"
					...
					<button
						type="button"
						onClick={() => {
							setStatFilter(statFilter === "vip" ? "all" : "vip");
						}}
						className={`rounded-xl border p-4 text-left transition-colors ${
							statFilter === "vip"
					...
					<button
						type="button"
						onClick={() => {
							setStatFilter(statFilter === "active" ? "all" : "active");
						}}
						className={`rounded-xl border p-4 text-left transition-colors ${
							statFilter === "active"
					...
					<button
						type="button"
						onClick={() => {
							setStatFilter(statFilter === "dormant" ? "all" : "dormant");
						}}
						className={`rounded-xl border p-4 text-left transition-colors ${
							statFilter === "dormant"
					...
					<div className="rounded-xl border border-neutral-200 bg-white p-4">
						...
						<p className="text-2xl font-bold text-blue-600">
							{formatLtv(customers.reduce((sum, c) => sum + c.totalSpent, 0))}
						</p>
					</div>
				</div>
			</div>
