...
export default function SalesListPage(): ReactElement {
	...
	// API 데이터 (USE_API=true이고 인증된 상태일 때만 fetch)
	const { isAuthenticated } = useAuth();
	const visitsQuery = useVisitsList(
		USE_API ? { date_from: startDate, date_to: endDate } : undefined,
	...
	// API 모드: 서버에서 date_from/date_to로 필터링 완료, mock 모드: Context 함수 사용
	const filteredSales = useMemo(() => {
		return USE_API ? apiSales : getSalesByDateRange(startDate, endDate);
	...
	const allSales = USE_API ? apiSales : sales;
	const selectedSale =
		selectedSaleId !== null ? allSales.find((s) => s.id === selectedSaleId) : null;
